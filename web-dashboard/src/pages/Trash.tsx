...
import { getTrashContent, restoreContent, clearTrash, deleteContent } from '../api/endpoints';
...
export default function Trash() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // error handled silently; items stays empty
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleClearAll = async () => {
    ...
    try {
      ...
      alert('Failed to clear trash. Please try again.');
    } finally {
      setIsClearing(false);
    }
  };
      ...
      <header className={styles.hero}>
        ...
        {items.length > 0 ? (
          <div className={styles.toolbar}>
            <label className={styles.selectAllLabel} title={allSelected ? 'Unselect all' : 'Select all'}>
              <input type="checkbox" checked={allSelected} onChange={toggleAll} />
              <span className={styles.srOnly}>Select all</span>
            </label>
            ...
            <button
              ...
              disabled={isClearing}
            >
              {isClearing ? 'Clearing…' : `Clear all (${items.length})`}
            </button>
          </div>
        ) : null}
      </header>
