...
class Summarizer:
    ...
    async def _execute_request(
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if response.status_code != 200:
                        raise APIConnectionError(f"API request failed with status {response.status_code}")
                    return self._extract_summary(response.json(), provider)
        ...
        raise SummarizationError(f"All {max_retries} retries failed. Last: {last_error}")
    ...
    async def ocr_screenshot(
        ...
        prompt = (
            ...
            "Then output all other text. Be thorough — include all readable text."
        )
        request_url, headers, payload, provider = self._build_request(
            prompt, image_bytes=image_bytes, image_media_type=media_type
        )
        payload["max_tokens"] = 600
        raw = await self._execute_request(request_url, headers, payload, provider, max_retries)
    ...
    async def generate_source_narrative(
        ...
        prompt = (
            ...
            f"Kept articles:\n{titles_block}\n\nTopics: {topics_block}{manual_block}"
        )
        request_url, headers, payload, provider = self._build_request(prompt)
        payload["max_tokens"] = 350
        return await self._execute_request(request_url, headers, payload, provider, max_retries)
